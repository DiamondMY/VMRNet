...
class VMRNet(nn.Module):
    ...
    def forward(self, data, mode):
        ...
        concatout = self.con_model(data['inner_feature'], data['video'], data.get('query'), mask=mask)

        if self.video_enc is not None:
            # d_emb = r_emb = v_emb = self.video_enc(data['video'], mask=mask)
            d_emb = r_emb = v_emb = self.video_enc(concatout, mask=mask)
        else:
            v_emb = data['video']
