...
from nncore.nn import DynamicBCELoss, GaussianFocalLoss
...
class BoundaryHead(nn.Module):
    ...
    def get_boundary(self, center_pred, window_pred, offset_pred):
        ...
        # 样本从大到小排序，并获取每个得分的实际位置
        scores, inds = torch.topk(center_pred, topk)

        # 中心秒数与对应偏移秒数相加，找到真实的中心（最小值>=0，不会向左偏移）
