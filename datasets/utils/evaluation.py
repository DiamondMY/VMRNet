...
def save_json(collected, label):
    f = open("sample_val_preds.jsonl", "w")

    for i in range(len(label)):
        data = {}
        data['qid'] = label[i]['qid']
        data['query'] = label[i]['query']
        data['vid'] = label[i]['vid']
        data['pred_relevant_windows'] = collected[i]['pred_relevant_windows']
        data['pred_saliency_scores'] = collected[i]['pred_saliency_scores']
        json.dump(data, f)
        if i != len(label) - 1:
            f.write('\n')
    f.close()